...
**바로 이어서 할 일 (우선순위 순)**
1. **실제 정보 채우기** — `index.html`에서 아래 임시값 교체:
   - 담임목사 성함 `○○○ 목사`
   - 예배 시간 (현재 09:00 / 11:00 / 19:30 등은 예시값)
   - 정확한 주소 `강원특별자치도 춘천시 ○○로 00`, 전화 `033-000-0000`, 이메일
   - 교회 소식(공지) 3건 내용/날짜
2. **사진 넣기** — `images/` 폴더에 `hero1~3.jpg`, `community.jpg`, `pastor.jpg` (아래 3-1 표 참고)
3. **설교 영상** — 유튜브(갈릴리마을TV `@TV-ew5ic`) 특정 영상 iframe 삽입 원하면 링크 준비
4. (원하면) 참고 사이트 chmchurch.org에서 마음에 든 요소 추가 반영 — ※ 그 사이트는 보안 인증서 문제로 클로드가 못 열었음. 원하는 부분을 말로 설명 필요.

**미해결/확인 필요**
- 담임목사님 실제 성함·사진, 실제 예배시간표, 정확한 주소 → 사용자 확인 대기
